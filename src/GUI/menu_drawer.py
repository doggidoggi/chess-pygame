...
class MainMenuDrawer:
    def __init__(self, window, settings):
        ...
        self.classic_ai_button = None
        self.classic_ai_button_surface = None
        self.self_learning_ai_button = None
        self.self_learning_ai_button_surface = None
    ...
    def __setup_buttons(self):
        width = WIDTH // 2
        height = HEIGHT // 8
        color = pygame.Color(227, 193, 111)
        x_axis = WIDTH / 2 - width / 2
        y_axis = height * 2.5
        self.multiplayer_button = pygame.Rect(x_axis, y_axis, width, height)
        self.multiplayer_button_surface = get_button(self.multiplayer_button, pygame.font.SysFont('Times New Roman', 20,
                                                                                                  bold=True),
                                                     self.button_font_color, color, 'Против друга', WIDTH, HEIGHT)
        y_axis = height * 4
        self.singleplayer_button = pygame.Rect(x_axis, y_axis, width, height)
        self.singleplayer_button_surface = get_button(self.singleplayer_button, pygame.font.SysFont('Times New Roman', 20,
                                                                                                    bold=True),
                                                      self.button_font_color, color, 'Одиночная Игра', WIDTH, HEIGHT)
        y_axis = height * 2.5
        self.classic_ai_button = pygame.Rect(x_axis, y_axis, width, height)
        self.classic_ai_button_surface = get_button(self.classic_ai_button, pygame.font.SysFont('Times New Roman', 20,
                                                                                                bold=True),
                                                    self.button_font_color, color, 'Классический ИИ', WIDTH, HEIGHT)
        y_axis = height * 4
        self.self_learning_ai_button = pygame.Rect(x_axis, y_axis, width, height)
        self.self_learning_ai_button_surface = get_button(self.self_learning_ai_button,
                                                          pygame.font.SysFont('Times New Roman', 20, bold=True),
                                                          self.button_font_color, color, 'Self-learning ИИ', WIDTH, HEIGHT)
    ...
    def draw(self):
        background_color = pygame.Color(184, 139, 74)
        self.GameScreen.fill(background_color)
        if self.in_main_menu:
            ...
            self.GameScreen.blit(self.quit_button_surface, (0, 0))
            self.GameScreen.blit(self.classic_ai_button_surface, (0, 0))
            self.GameScreen.blit(self.self_learning_ai_button_surface, (0, 0))
        pygame.display.flip()
