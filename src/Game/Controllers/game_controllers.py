...
class Game:
    def __init__(self, window, settings):
        ...
        self.AI_move = None
        self.ai_mode = AI_MODE_CLASSIC
        self.ai_checkpoint_path = None
        self.ai_fallback_to_classic = True
        self._ai_mode_warning_printed = False
    ...
    def against_ai(self, ai_mode=AI_MODE_CLASSIC, checkpoint_path=None, fallback_to_classic=True):
        pygame.event.clear()
        self.ai_mode = ai_mode
        self.ai_checkpoint_path = checkpoint_path
        self.ai_fallback_to_classic = fallback_to_classic
        self._ai_mode_warning_printed = False
        self.config.change_square(8)
        self.ChessBoard = WithoutTimerChess()
        self.ValidMoves = self.ChessBoard.getValidMoves()
        ...
        while self.mainloop:
            self.human_turn = ((self.player_one and self.ChessBoard.whiteMove) or
                               (self.player_two and not self.ChessBoard.whiteMove))
            if not self.game_end:
                self.__check_for_game_end()
            self.__update_mouse_position()
            self.__handle_events()
    ...
    def __choose_ai_move(self):
        if self.ai_mode == AI_MODE_SELF_LEARNING and not self._ai_mode_warning_printed:
            if not has_trained_ai(self.ai_checkpoint_path):
                print('Self-learning AI checkpoint not found. Falling back to classic AI.')
                self._ai_mode_warning_printed = True
        ...
        return find_best_move_with_mode(
            ...
            checkpoint_path=self.ai_checkpoint_path,
            fallback_to_classic=self.ai_fallback_to_classic,
        )

    def __update_mouse_position(self):
        mouse_x, mouse_y = pygame.mouse.get_pos()
        if mouse_x <= WIDTH and mouse_y <= HEIGHT:
            self.mouse_position = [angle // self.config.SQUARE_SIZE for angle in (mouse_x, mouse_y)]
    ...
    def __handle_events(self):
        ...
        if not self.game_end and not self.human_turn:
            self.AI_move = self.__choose_ai_move()
            if self.AI_move is not None:
                apply_ai_move(self.ChessBoard, self.AI_move)
    ...
    def __handle_lbm_pressed(self):
        ...
        if (self.clicked_piece and ((self.clicked_piece.piece_color == WHITE and self.ChessBoard.whiteMove)
                                    or (self.clicked_piece.piece_color == BLACK and not self.ChessBoard.whiteMove))
                or self.PlayerClicksLog):
            if (self.clicked_piece and ((self.clicked_piece.piece_color == WHITE and self.ChessBoard.whiteMove)
                                        or (self.clicked_piece.piece_color == BLACK and
            ...
            if len(self.PlayerClicksLog) == 2:
                ...
                move = Move((self.PlayerClicksLog[0][0], self.PlayerClicksLog[0][1]),
                            (self.PlayerSelectedCell[0], self.PlayerSelectedCell[1]),
                            self.ChessBoard)
